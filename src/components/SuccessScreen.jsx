import React from 'react';

const SuccessScreen = ({ status, onRetry }) => {
    const isSyncing = status === 'syncing';
    const isError = status === 'error';
    const isSuccess = status === 'success' || status === 'idle'; // idle fallback

    return (
        <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4 animate-slide-up">
            {/* dynamic Icon based on status */}
            <div className="relative mb-8">
                {isSyncing ? (
                    <div className="w-24 h-24 bg-indigo-100 rounded-full flex items-center justify-center border-4 border-indigo-500 border-t-transparent animate-spin">
                    </div>
                ) : isError ? (
                    <div className="w-24 h-24 bg-gradient-to-br from-red-400 to-red-600 rounded-full flex items-center justify-center shadow-xl shadow-red-500/30">
                        <svg className="w-14 h-14 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </div>
                ) : (
                    <>
                        <div className="w-24 h-24 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-full flex items-center justify-center shadow-xl shadow-emerald-500/30">
                            <svg className="w-14 h-14 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path className="animate-checkmark" strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                            </svg>
                        </div>
                        <div className="absolute inset-0 w-24 h-24 bg-emerald-400/20 rounded-full animate-ping"></div>
                    </>
                )}
            </div>

            {/* Success Message */}
            <h2 className="text-3xl md:text-4xl font-bold text-slate-800 mb-4">
                {isSyncing ? 'Synchronizing...' : isError ? 'Sync Failed!' : 'Registration Successful!'}
            </h2>

            <p className="text-lg text-slate-600 mb-6 max-w-md">
                {isSyncing 
                    ? "We are saving your details to our records. Please don't close this window."
                    : isError 
                    ? "Something went wrong while saving. Your data is still safe in this browser."
                    : "Thank you for registering for GNH Community Yatra:Hampi 2026."}
            </p>

            {isError && (
                <button 
                    onClick={onRetry}
                    className="btn-primary px-8 py-3 mb-8"
                >
                    Try Again
                </button>
            )}

            {!isError && (
                <div className="space-y-4 w-full max-w-md">
                    <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-6">
                        <div className="flex items-start space-x-3 text-left">
                            <div className="flex-shrink-0">
                                <svg className="w-6 h-6 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                            </div>
                            <div>
                                <h4 className="font-semibold text-indigo-800 mb-1">Join WhatsApp Group</h4>
                                <p className="text-sm text-indigo-700 mb-4">
                                    Please join our official Yatra WhatsApp group for all updates and community coordination.
                                </p>
                                <a 
                                    href="https://chat.whatsapp.com/FvH2w48R9iW82gbHM9Z4MW" 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                    className="inline-flex items-center space-x-2 bg-[#25D366] text-white px-5 py-3 rounded-lg font-bold hover:bg-[#128C7E] transition-colors shadow-md shadow-green-200"
                                >
                                    <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
                                    </svg>
                                    <span>Join WhatsApp Group</span>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            )}

            {/* Decorative element */}
            <div className="mt-10 flex items-center space-x-2 text-slate-400">
                <span className="text-2xl">🙏</span>
                <span className="text-sm font-medium">Hare Krishna</span>
                <span className="text-2xl">🙏</span>
            </div>
        </div>
    );
};

export default SuccessScreen;
